...
export class Lfm25OnnxWebGpuAdapter implements VLMAdapter {
  id = 'lfm25-onnx-webgpu';
  label = 'LiquidAI LFM2.5-VL-450M ONNX';
  private ready = false;
  ...
  async load() {
    if (this.ready && this.model && this.processor) {
      this.emitStatus({ stage: 'ready', message: 'LiquidAI ONNX runtime is ready.' });
      return;
    }
    ...
    this.loadingPromise = (async () => {
      ...
      if (!support.supported) {
        throw new Error(support.reason ?? 'This browser cannot run the LiquidAI ONNX model.');
      }
  ...
  async transcribe(image: RasterizedRow): Promise<VLMResult> {
    ...
    if (!this.model || !this.processor || !this.transformers) {
      throw new Error('The LiquidAI ONNX model is not ready yet.');
    }
