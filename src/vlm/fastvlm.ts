...
const TRANSCRIPTION_PROMPT =
  '<image>Read the handwritten mathematical expression in this image. Return only a single LaTeX expression with no prose, no markdown, and no explanation.';
...
export class FastVlmOnnxWebGpuAdapter implements VLMAdapter {
  id = 'fastvlm-onnx-webgpu';
  label = 'FastVLM 0.5B ONNX';
  private ready = false;
  ...
  async load() {
    if (this.ready && this.model && this.processor) {
      this.emitStatus({ stage: 'ready', message: 'FastVLM ONNX runtime is ready.' });
      return;
    }
    ...
    this.loadingPromise = (async () => {
      ...
      if (!support.supported) {
        throw new Error(support.reason ?? 'This browser cannot run the FastVLM ONNX model.');
      }
    ...
    return this.loadingPromise.catch((error) => {
      this.emitStatus({
        stage: 'error',
        message: error instanceof Error ? error.message : 'Failed to load the FastVLM model.',
      });
      throw error;
    });
  }
  ...
  async transcribe(image: RasterizedRow): Promise<VLMResult> {
    ...
    if (!this.model || !this.processor) {
      throw new Error('The FastVLM ONNX model is not ready yet.');
    }
    ...
    this.emitStatus({
      stage: 'loading',
      message: `Recognizing formula with FastVLM (${MAX_NEW_TOKENS} token cap)...`,
    });
    ...
    const decoded = this.processor.batch_decode(
      outputs.slice(null, [inputs.input_ids.dims.at(-1) ?? 0, null]) as Parameters<
